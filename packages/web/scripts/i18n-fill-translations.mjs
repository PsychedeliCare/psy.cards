...
function applyStaticTranslations(locale) {
  ...
  const enNotes = readJson("packages/shared/i18n/content/en/combo-notes.json");
  const enStatus = readJson("packages/shared/i18n/content/en/status-definitions.json");
  const enClass = readJson("packages/shared/i18n/content/en/class-descriptions.json");

  writeJson(`packages/shared/i18n/content/${locale}/drug-labels.json`, staticData.drugLabels);
  ...
  writeJson(`packages/shared/i18n/content/${locale}/status-definitions.json`, statusDefs);
  ...
  writeJson(`packages/shared/i18n/content/${locale}/class-descriptions.json`, classDesc);

  return { enNotes, localeDir: `packages/shared/i18n/content/${locale}` };
}
...
async function main() {
  ...
  for (const locale of locales) {
    ...
    if (translateDrugs) {
      const enDrugsDir = path.join(root, "packages/shared/i18n/content/en/drugs");
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        await translateDrugFile(`packages/shared/i18n/content/en/drugs/${file}`, outPath, locale);
        await new Promise((r) => setTimeout(r, BATCH_DELAY_MS));
      }
      console.log();
    } else {
