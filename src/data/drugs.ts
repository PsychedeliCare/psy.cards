...
import { columnByKey } from "./config";

type DrugsMap = Record<string, Drug>;
const drugs = drugsData as unknown as DrugsMap;

/** Column keys that map to a drug class rather than a single `drugs.json` entry. */
...
/** Canonical drug-id used when a column key doesn't exist in drugs.json. */
const keyAlias: Record<string, string> = {
  dxm: "dextromethorphan",
  "ghb/gbl": "ghb",
};

export function getDrugByKey(key: string): Drug | undefined {
  const direct = drugs[key];
  if (direct) return direct;
  const aliased = keyAlias[key];
  if (aliased) return drugs[aliased];
  return undefined;
}

export function isDrugClass(key: string): boolean {
  return key in classDescriptions;
}

export function getClassDescription(key: string): string | undefined {
  return classDescriptions[key];
}

export function getDisplayName(key: string): string {
  return columnByKey.get(key)?.label ?? key;
}
